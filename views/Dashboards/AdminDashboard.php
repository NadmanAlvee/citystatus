...
function ajax(method,url,data,callback){
    ...
    xhr.onload=function(){
        if(xhr.status>=200 && xhr.status<300) callback(JSON.parse(xhr.responseText));
        else console.error('Request failed:',xhr.responseText);
    };
    ...
    ajax('GET',USER_API+'getUsers',null,function(data){
        list.innerHTML=data.map(u=>`
            <div class="item">
                ...
                <div>
                    <!-- Redirect to manage-users page with query param for user_id -->
                    <button class="btn" onclick="window.location.href='/citystatus/manage-users?user_id=${u.user_id}'">Update</button>
...
function deleteArea(areaId) {
    ...
    ajax('POST', POST_API + 'deleteArea', { area_id: areaId }, function(res) {
        if (res.success) {
            loadAreas();
        } else {
...
function escapeHtml(str) {
    ...
    // Replaces special characters using the map above
